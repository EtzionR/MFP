...
rmse = lambda mat1, mat2: torch.sqrt(torch.mean(torch.pow(mat1 - mat2, 2))).float().item()
mae = lambda mat1, mat2: torch.mean(torch.abs(mat1 - mat2)).float().item()
gap = lambda ary: INF if len(ary) < LEN else (ary[-2] - ary[-1])/(ary[0] - ary[-1])
...
class FP:
    ...
    def __init__(self, index=False, iters=50, eps=1e-3, delta_func=rmse, early_stop=False):
        """
    ...
    def build_a_hat(self, nodes_num, edges):
        ...
        edges = (edges if edges.size()[0] == TWO else edges.T).to(self.device)

        # add in-links to edge index matrix (links between node to himself)
        eye   = torch.arange(nodes_num).to(self.device)
    ...
    def plot(self, size=10):
        ...
        plt.plot(axe, self.delta, color='r', label='Delta')
        plt.xlabel('Iterations', fontsize=14)
        plt.ylabel('Delta', fontsize=14)
        plt.ylim(0, self.delta[0] * 1.01)
